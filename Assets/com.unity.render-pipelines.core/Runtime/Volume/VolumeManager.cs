...
namespace UnityEngine.Experimental.Rendering
{
    ...
    public sealed class VolumeManager
    {
        ...
        readonly List<FakeCollider> m_TempFakeColliders;
        ...
        VolumeManager()
        {
            ...
            m_TempFakeColliders = new List<FakeCollider>(8);
        ...
        public void Update(VolumeStack stack, Transform trigger, LayerMask layerMask)
        {
            ...
            foreach (var volume in volumes)
            {
                ...
                // If volume isn't global and has no FakeCollider, skip it as it's useless
                var FakeColliders = m_TempFakeColliders;
                volume.GetComponents(FakeColliders);
                if (FakeColliders.Count == 0)
                    continue;

                // Find closest distance to volume, 0 means it's inside it
                float closestDistanceSqr = float.PositiveInfinity;

                foreach (var collider in FakeColliders)
                {
                    if (!collider.enabled)
                        continue;
